'''
    orangev1.py; orange_code version 1, a very early version, is capable of running basic orange_code programs.
...
#This is the original code, copied directly from repl.it; many parts of the code (e.g. "if true:") are
#added to make it more convenient to read in replit and don't change the functionality of the code.
#More versions (such as terminal and executible) will be added later.

true=True #to avoid replit warnings
####SETUP####
if true:
	import time
	import re
...
if true:
	...
	#function
	if true: #to make it easier to condense all this crap :)
	...
	if true:
		...
		def wait(_,arg,__):
			casted=arg.getString('cast').run(strObj('number'),arg)
			if type(casted) is numObj:
				if shouldPrint: #only false when testing
		...
		stringObj=None #BC REPLIT KEEPS GIVING ME A RED X MARK SAYING STROBJ IS UNDEFINED AAAAA
		...
		def runCode(code):
			...
			runObj.code=code
			runObj.run(noneObj())
####OTHER TESTING THINGIES####
#baseObj=codeObj()
#baseObj.code=''
#baseObj.run(codeObj())
#print()
#btw dont think [].whatever would work, fix that maybe
####TESTING####
testVals = [
['#a=1.5;wait~a;print~a',['1.5']],
...
["print~'hello world'",['hello world']],
["#a=(print~'hello world';0);print~a",['hello world','0']],
["#a=[print~'hello world';0];a~none;print~a.0",['hello world','hello world','0']],
["#a={print~'hello world';0};a~none;print~a.0",['hello world','0']],
['print~2**3;print~10//3;print~65%3',['8','3','2']],
['print~1==1;print~2+2==5',['true','false']],
['print~1>0;print~0>1',['true','false']],
['print ~ "    a   "',['    a   ']],
['1,print~0.cast~"boolean";print~.0.cast~"boolean";',['false','true']],
['$`#"a"=0;print~$`."a"',['0']],
['while~[2,{print~"hi";$$#0-=1}]',['hi','hi']],
['for~[[2,3,4],{print~@}]',['2','3','4']],
['#a={return~65};print~a~a',['65']],
['print~[1,2,3,4]`."len"~{}',['4']],
['print~"\\t\\n \\a\\\\"',['\t\n a\\']]
]
if testingMode:
	...
	if testsFailed==0:
		...
		while True:
			...
			for tab in testVals:
				...
				try:
					runCode(tab[0])
					if printedVals!=tab[1]:
						testsFailed2.append(tab)
						print(colored("Test "+str(len(testsFailed2))+" failed.","red"))
						print("Code:",tab[0])
						print("Expected prints:",tab[1])
						print("Actual prints:",printedVals)
				except BaseException:
					testsFailed2.append(tab)
					print(colored("Test "+str(len(testsFailed2))+" errored.","red"))
					print("Code:",tab[0])
					print("Expected prints:",tab[1])
			print("Which errored tests would you like to see the verbose output for?")
			num = input()
			verbose = True
			shouldPrint = True
			runCode(testsFailed2[int(num)-1][0])
			verbose = False
			shouldPrint = False
	shouldPrint=True
####RUN CODE####
